...
public:
  enum {
    ...
    WS = 8, MUL = 9, DIV = 10, ADD = 11, SUB = 12, ID = 13
  };

  enum {
    RuleProg = 0, RuleStatement = 1, RuleAssignment = 2, RuleNumber = 3, 
    RuleExpr = 4
  };

  explicit ArithmeticParser(antlr4::TokenStream *input);

  ArithmeticParser(antlr4::TokenStream *input, const antlr4::atn::ParserATNSimulatorOptions &options);

  ~ArithmeticParser() override;
